...
export class SubCategoryDetailsComponent implements OnInit {
  ...
  load() {
    ...
    if (this.categoryId !== 'add') {
      ...
      if (this.parentId) {
        this.categoryService.getCategory(this.parentId).subscribe(data => {
          if (data) {
            this.category = {
              CategoryId: '',
              Name: '',
    ...
    if (this.category.CategoryId && this.category.CategoryId.length > 5) {
      ...
      this.categoryService.add(this.category).subscribe(data => {
        if (data && data.CategoryId) {
          this.messageService.add({ severity: 'success', summary: 'Category details saved.', detail: '' });
          if(this.categoryId === 'add')
          this.router.navigate([`/admin/dashboard/sub-category/${data.CategoryId}/${this.parentId}`])
        }
      });
    }
  }
  back() {
    this.router.navigate([`/admin/dashboard/category/${this.category.ParentId}`]);
  }
